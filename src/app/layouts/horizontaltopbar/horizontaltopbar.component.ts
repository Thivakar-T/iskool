import { Component, OnInit, AfterViewInit, Inject, Output, EventEmitter } from '@angular/core';
...
import { SuperAdminService } from '../../pages/super-admin.service';
...
export class HorizontaltopbarComponent implements OnInit, AfterViewInit {
  ...
  @Output() callSideNav = new EventEmitter();
  ...
  ngOnInit(): void {
    ...
    this.agentObj = JSON.parse(localStorage.getItem('currentUser')).data.loginObj.agentObj;
    this.item = JSON.parse(localStorage.getItem('subItems'));
    ...
    // setInterval(() =>{
    //   this.isRightNavOpen = JSON.parse(localStorage.getItem('isRightNavOpen'));
    // }, 1000)
  }

  get k() { return this.agentForm.controls; }
...
//   getNotification(){
//     this._notificationService.getNotification().pipe(first()).subscribe(res =>{
//       this.notificationArr = res.data.slice(0,3);
//     })
//  }

 changeAgent(content){
   var isProductOwner = JSON.parse(localStorage.getItem('isProductOwner')); 
   if(isProductOwner){
...
//  getAgent() {
//   this._agentsService.getAgent().pipe(first()).subscribe(res => {
//     var agent = res.data;
//     this.agentArr = agent.filter(person => person.id != this.agentObj.agentId);
//   })
// }
